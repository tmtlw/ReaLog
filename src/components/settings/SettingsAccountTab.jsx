...
const SettingsAccountTab = ({ localSettings, setLocalSettings, themeClasses, t }) => {
    const profileRef = useRef(null);
    const coverRef = useRef(null);

    const handleImageUpload = async (file, type: 'profile' | 'cover') => {
        try {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div>
                ...
                <Input themeClasses={themeClasses} value={localSettings.userName || ''} onChange={(e) => setLocalSettings(prev => ({ ...prev, userName: e.target.value }))} placeholder={t('settings.account_hint_name')} />
            </div>
            ...
            <div>
                ...
                <select value={localSettings.language || 'hu'} onChange={(e) => setLocalSettings(prev => ({ ...prev, language: e.target.value }))} className={`w-full rounded-lg px-4 py-2 border focus:ring-2 focus:outline-none transition-all ${themeClasses.input} ${themeClasses.bg}`}>
            ...
            <div>
                ...
                <Input themeClasses={themeClasses} type="password" value={localSettings.adminPassword || ''} onChange={(e) => setLocalSettings(prev => ({ ...prev, adminPassword: e.target.value }))} placeholder={t('settings.admin_password_hint')} />
