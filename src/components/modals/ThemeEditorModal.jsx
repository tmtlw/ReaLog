...
const WeatherPreview void, t }> = ({ pack, currentPack, onSelect, t }) => {
...
const EmojiPreview void, t }> = ({ style, currentStyle, onSelect, t }) => {
    const samples = ['🙂', '🔥', '🚀', '🌈', '🎉'];
    return (
        <div
...
const PreviewCard = ({ classes, label, active, onClick, noTranslate, t }) => (
...
const ThemeEditorModal void;
    data;
    setData.Dispatch<React.SetStateAction<AppData>>;
    currentTheme;
    setCurrentTheme: (t) => void;
    themeClasses;
    t: (key) => string;
    ...
    // Weather State
    const [weatherPack, setWeatherPack] = useState(data.settings?.weatherIconPack || 'outline');

    // Emoji State
    const [emojiStyle, setEmojiStyle] = useState(data.settings?.emojiStyle || 'native');
    const [isSaving, setIsSaving] = useState(false);

    const [fontSearch, setFontSearch] = useState('');
    const fontFileInput = useRef(null);
    ...
    const handlePreviewSecret = (themeId) => {
    ...
    const handleSaveCustom = () => {
        const config = { base: customBase, accent: customAccent, customBg: customBg || undefined };
        setData(prev => ({
            ...prev,
            settings: {
                ...prev.settings,
