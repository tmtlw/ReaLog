...
const StatsView = ({ entries, themeClasses, t, savedLayout, onSaveLayout, weatherPack = 'outline', emojiStyle = 'native' }) => {
    ...
    const moodCounts<string, number> = {};
    let totalMoods = 0;

    // 2. Locations
    const locations = new Set<string>();
    const locationCounts<string, number> = {};
    ...
    const dayActivity<string, number> = {}; // "YYYY-MM-DD" -> count
    ...
    const weatherMoods<string, Record<string, number>> = {}; // { 'Clouds': { '🙂': 5 } }
    const dayMoods<number, Record<string, number>> = {};
    ...
    const getTopMood = (map<string, number>) => {
        let top = '';
        let max = 0;
        let total = 0;
    ...
    // Reordering Helpers
    const moveWidget = (index, direction: 'up' | 'down') => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 h-full">
            ...
            <Card themeClasses={themeClasses} className="p-6 h-full">
                ...
                <div className="space-y-3">
                    {Object.keys(weatherMoods).length === 0 ? <p className="opacity-50 text-sm text-center">{t('stats.no_data')}</p> .entries(weatherMoods).map(([cond, moods]) => {
            ...
            <Card themeClasses={themeClasses} className="p-6 h-full">
                ...
                <div className="space-y-3">
                    {Object.keys(dayMoods).length === 0 ? <p className="opacity-50 text-sm text-center">{t('stats.no_data')}</p> .entries(dayMoods).sort((a,b) => Number(a[0]) - Number(b[0])).map(([dayIdx, moods]) => {
                            const { mood, percent } = getTopMood(moods);
                            return (
