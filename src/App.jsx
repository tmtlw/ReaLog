...
import {
  ...
  Image, Cloud, Calendar, List, Grid,
  Smile, Settings, Info, Server, MapPin, Eye, EyeOff, Palette, Search, ChevronLeft, Map,
  ...
  Globe, Images, Layers, Shield, ShieldAlert, Clock, Bold, Italic, Underline, Link, AlignLeft,
...
export default function App() {
  ...
  const [logoEmoji, setLogoEmoji] = useState(null);
  ...
  // Views
  const [globalView, setGlobalView] = useState('none');
  ...
  const weatherPack = data.settings?.weatherIconPack || 'outline';
  const emojiStyle = data.settings?.emojiStyle || 'native';
  ...
  useEffect(() => {
    const init = async () => {
        ...
        // Load data from PHP API (loadData is now async)
        const local = await StorageService.loadData();
        ...
        // We are always in "Server Mode" now because we use PHP API
        setServerMode(true);
        ...
        // Setup Auto Sync
        // (Assuming existing logic works with serverSave alias)
    };
    init();
  }, []);
  ...
  useEffect(() => {
      ...
      // Standard Themes
          ...
          classes = isDark ? THEMES.dark .light;
      }
  ...
  const startNewEntry = () => {
      ...
      // Initialize responses based on priority Template -> Active Questions -> Random 4
      ...
      const initialResponses<string, string> = {};
  ...
  const saveEntry = () => {
      ...
      const newEntry = { ...currentEntry, isDraft: false };
  ...
  const duplicateEntry = (e) => {
      const newEntry<Entry> = {
          ...e,
          id: crypto.randomUUID(),
          timestamp.now(), // Set to now for the new copy
  ...
  const filteredEntriesAll = useMemo(() => {
      ...
      if (searchQuery) {
          ...
          entries = entries.filter(e =>
              ...
              Object.values(e.responses).some((r) => typeof r === 'string' && r.toLowerCase().includes(q))
          );
      }
              ...
              {e.tags.map(tag => {
                  ...
                  return (
                    <button
                        ...
                        style={{
                            backgroundColorToBgColor(tag, isDarkTheme ? 'dark' : 'light'),
                            colorToColor(tag, isDarkTheme ? 'dark' : 'light')
                        }}
                    >
                        #{tag}
                    </button>
